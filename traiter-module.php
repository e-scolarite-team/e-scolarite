...
class Traiter extends BaseAdmin
{
    ...
    {
        echo <<<EOC
    <h3 class="titre ombre">Traiter les réclamations d'inscription en module</h3>
EOC;
        if($this->msgi!='')
        {
        ...
        else
        {
            ...
            <div class="detail-contenu">
                <p>Il y a <strong>$nbr</strong> réclamation d'inscription en module non traitées.</p>
            </div>
            <hr style="border-top: #66CE88 solid 2px;margin:0 18%;"/>
            <div class="detail-contenu">
                <form action="traiter-module-t.php" method="post">
...
EOC;
    ...
    public function verification()
    {
        ...
        $err=null;
        $stmt=null;
        $req = "SELECT `rec`.`numero`,`rec`.`cne`,DATE_FORMAT(`rec`.`dateenvoi`,GET_FORMAT(DATE,'EUR')) AS `dateenvoi`,`et`.`nom`,`et`.`prenom` FROM `reclamationmodule` AS `rec` INNER JOIN `etudiant` AS `et` ON `rec`.`cne` = `et`.`cne`WHERE isnull(`rec`.`etat`);";
