...
class Traiter extends BaseAdmin
{
    ...
    {
        return "Historique des réclamations de changement de module";
    }
    ...
    {
        echo <<<EOC
    <h3 class="titre ombre">Historique des réclamations de changement de module</h3>
EOC;
        if($this->msgi!='')
        {
...
EOC;
    ...
    public function verification()
    {
        ...
        $err=null;
        $stmt=null;
        $req = "SELECT `rec`.`numero`,`rec`.`cne`,DATE_FORMAT(`rec`.`dateenvoi`,GET_FORMAT(DATE,'EUR')) AS `dateenvoi`,`rec`.`remarque`,`et`.`nom`,`et`.`prenom` FROM `reclamationchanger` AS `rec` INNER JOIN `etudiant` AS `et` ON `rec`.`cne` = `et`.`cne` WHERE NOT isnull(`rec`.`etat`) ORDER BY `rec`.`numero` DESC LIMIT 0,100;";
