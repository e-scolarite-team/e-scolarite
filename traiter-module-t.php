    ...
    public function traiter_donnees()
    {
        ...
        {            
            ...
            try
            {
                ...
                {
                    $req = "UPDATE `reclamationmodule` SET `remarque` = ?,`etat` = ? WHERE `numero`=?;";
                    if($stmt = db_engine::preparer($req))
                    {
                        if(isset($rec['decision']))
                        {
                            ...
                            $req = "DELETE FROM `reclamationmoduleaff` WHERE `num`=?;";
                            if($stmt = db_engine::preparer($req))
                            {
                            ...
                            $req = "DELETE FROM `reclamationmodulerec` WHERE `num`=?;";
                            if($stmt = db_engine::preparer($req))
                            {
                                $stmt->bind_param("i",$num);
                                $stmt->execute();
            ...
            redirectToPage("traiter-module.php?i=100&nb=$i");
            exit();
        }
