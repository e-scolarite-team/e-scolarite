...
{    
    ...
    {
        echo <<<EOC
    <h3 class="titre ombre">Réclamation d'inscription en module</h3>
    <div class="detail-contenu" >
    <p>
        Cette page est consacrée pour effectuer une réclamation d'inscription en module.
    </p>
    <p>
        Pour réaliser la demande commencer par choisir le module affiché puis le module que vous réclamez ensuite cliquer sur envoyer.  
    </p>
    </div><!-- .detail-contenu -->
...
EOC;
        ...
        echo <<<EOC
    <div class="detail-contenu" >
        <form action="reclamation-module-t.php" method="post">
        <fieldset>
...
EOC;
    ...
    {
        parent::verification();
        $req = "SELECT `modules`.`mid` FROM `modules` INNER JOIN `session_module` ON `modules`.`sess_mod`=`session_module`.`sid` AND `session_module`.`etat`=1 ORDER BY `modules`.`mid`;";
