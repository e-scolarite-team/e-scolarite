...
class Reponse extends BaseEtudiant
{
    ...
    {
        return "Statut des réclamations d'inscription en module";
    }
    ...
    {
        echo <<<EOC
    <h3 class="titre ombre">Statuts des  réclamations d'inscription en module</h3>
EOC;
        ...
        else
        {
            ...
            <div class="detail-contenu">
                <p>Vous avez <strong>$nbr</strong> réclamations d'inscription en module.</p>
            </div>
...
EOC;
    ...
    public function verification()
    {
        ...
        $req = "SELECT `rec`.`numero`,DATE_FORMAT(`rec`.`dateenvoi`,GET_FORMAT(DATE,'EUR')) AS `dateenvoi`,`rec`.`remarque`,`rec`.`etat` FROM `reclamationmodule` AS `rec`  WHERE `rec`.`cne` = ?  AND NOT isnull(`rec`.`etat`) ORDER BY `rec`.`numero` DESC;";
